...
#define ADC_OFFSET				691
// defines mA for one adc change
// be careful, the numerator of the fraction has to be smaller than 65535
// otherwise the int32_t calulation can be get an overflow
#define ADC_TO_CURRENT			7697 / 9999
// defines mV for one adc change
#define ADC_TO_VOLTAGE			404
...
void showCurrentAndTime(const int32_t remainingCapacity)
{
	...
	if (current < 0)
	{
		...
		if (remainingCapacity > 0 && remainingHours <= MAX_REMAINING_HOURS)
		{
